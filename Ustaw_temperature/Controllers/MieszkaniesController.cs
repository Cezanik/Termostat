...
namespace Ustaw_temperature.Controllers
{
    public class MieszkaniesController : Controller
    {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Edit(int id, [Bind("Id,LiczbaOkien,LiczbaPokoi,BazowaTemperatura")] string Uzytkownik2, Mieszkanie mieszkanie)
        {
            if (id != mieszkanie.Id)
            {
                return NotFound();
            }
            var mieszkanie1 = await _context.Mieszkanie.FindAsync(id);
            if (mieszkanie1 != null)
            {
